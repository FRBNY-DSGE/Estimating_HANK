...
function impulse_responses(m::AbstractDSGEModel, system::RegimeSwitchingSystem{S}, horizon::Int;
    ...
    if start_reg != reg_fcast_start
        ...
        post_cond_regs = subtract_quarters(get_setting(m, :date_forecast_start), get_setting(m, :date_conditional_end))
        ...
        m <= Setting(:date_conditional_end, iterate_quarters(get_setting(m, :date_forecast_start), post_cond_regs))

        cond_end = iterate_quarters(date_conditional_end(m), 1)
...
function impulse_responses(system::System{S}, horizon::Int, permute_mat::AbstractMatrix{T},
    ...
    system = zero_system_constants(system) # Set constant system matrices to 0
    s₀ = zeros(S, nstates)
