...
"""
function n_forecast_draws(m::AbstractDSGEModel, input_type::Symbol; filestring_addl::Vector{String} = Vector{String}(undef, 0))
    ...
    if prod == :trend
        ...
        if regime_switching
            if haskey(get_settings(m), :time_varying_trends) && get_setting(m, :time_varying_trends)
                ...
                return (ndraws, nvars, subtract_quarters(end_date, start_date)+1)
...
function get_trend_dates(regime_dates::Dict{Int64, Date}, trends::AbstractArray,
    ...
    indings = [iterate_quarters(date_start, i) for i in 0:date_length-1]
