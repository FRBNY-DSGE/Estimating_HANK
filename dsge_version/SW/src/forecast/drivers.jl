        ...
        elseif get_setting(m, :sampling_method) == :SMC
            if input_type in [:mode, :mode_draw_shocks]
                ...
                elseif (occursin("smc_paramsmode", input_file_name) &&
                    ...
                    params = if typeof(cloud) <: Union{Cloud,SMC.Cloud}
...
- `filestring_addl::Vector{String}`: in case you need to pass in an addition to the standard input file names
...
function forecast_one(m::AbstractDSGEModel{Float64},
                      ...
                      shock_qtrs::Vector{UnitRange{Int}} = [-1:1], use_changing_systems::Bool = false, filestring_addl::Vector{String} = Vector{String}(undef, 0))
