...
* `observable_mappings::OrderedDict{Symbol,Observable}`: A dictionary that
  ...
  units. The package will fetch data from the Federal Reserve Bank of St. Louis's
...
buted to steady-state inflation.",
    ...
    for key in get_setting(m, :proportional_antshocks)
        propant_tex_label = detexify(key) == key ? string(key) : "\\" * string(detexify(key))
    ...
    for (sh, ant_num) in get_setting(m, :antshocks)
        ant_tex_label = detexify(sh) == sh ? string(sh) : "\\" * string(detexify(sh))
        for i in 1:ant_num
...
function model_settings!(m::Model1002)
    ...
    if subspec(m) in ["ss16", "ss17"]
        m <= Setting(:laborshare_base_period, quartertodate("1964-Q1"), "Base year for labor share series to provide an initial condition")
    end
