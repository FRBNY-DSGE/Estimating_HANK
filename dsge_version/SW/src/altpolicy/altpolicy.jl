...
- `eqcond::Function`: a version of `eqcond` which computes the equilibrium
  condition matrices under the alternative policy. Like `eqcond`, it should
  take in one argument of mutable struct `AbstractDSGEModel` and return the `Γ0`,
  `Γ1`, `C`, `Ψ`, and `Π` matrices.

- `solve::Function`: a version of `solve` which solves the model under the
  alternative policy. Like `solve`, it should take in one argument of mutable
