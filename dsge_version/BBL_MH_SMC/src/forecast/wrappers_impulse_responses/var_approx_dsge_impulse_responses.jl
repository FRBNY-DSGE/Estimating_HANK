...
```
computes the impulse responses of a VAR(p) approximation to a 
...
function impulse_responses(m::AbstractDSGEModel, paras::Union{Vector{S}, Matrix{S}},
    ...
    if isa(m, AbstractDSGEModel)
        ...
        update!(_dsgevar, lags = lags, observables = observables)
    else
        _dsgevar = m
...
function impulse_responses(m::AbstractDSGEVARModel, paras::Union{Vector{S}, Matrix{S}},
                           ...
                           verbose::Symbol = :high) where {S<:Real}
    update!(m, lags = lags, observables = observables, shocks = shocks)
    ...
    if create_meansbands
        ...
        if !test_meansbands
            ...
            var_names = save_as_DSGE ?
                Symbol("_" * join(string.(map(x -> detexify(x), observables)), "_") * "_") : Symbol("_")
