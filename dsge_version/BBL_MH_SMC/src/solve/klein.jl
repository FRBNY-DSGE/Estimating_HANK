       ...
       # m <= Setting(:LOMstate, Matrix{Float64}(undef,0,0))
        ...
        if get_setting(m,:linearize_heterogeneous_block)
            if get_setting(m,:load_bbl_posterior_mean)
                println("saving")
#=
                CSV.write(rawpath(m, "estimate", "/post/DSGE_LOM_Mat_v3.csv"),Tables.table(get_setting(m,:LOMstate)))
                CSV.write(rawpath(m, "estimate", "/post/DSGE_S2C_Mat_v3.csv"),Tables.table(get_setting(m,:State2Control)))

=#

             else
#=
              CSV.write(rawpath(m, "estimate", "prior/DSGE_LOM_Mat_v3.csv"),Tables.table(get_setting(m,:LOMstate)))
              CSV.write(rawpath(m, "estimate", "prior/DSGE_S2C_Mat_v3.csv"),Tables.table(get_setting(m,:State2Control)))
              #CSV.write("../../../../tables/DSGE_LOM_Mat_v3.csv",Tables.table(get_setting(m,:LOMstate)))
              #CSV.write("../../../../tables/DSGE_S2C_Mat_v3.csv",Tables.table(get_setting(m,:State2Control)))
=#
            end
            ...
            gx, hx, eu = klein(m)
#=
            CSV.write("../../../../DSGE_LOM_Reduced_Mat_v3.csv",Tables.table(get_setting(m,:LOMstate)))
            CSV.write("../../../../DSGE_S2C_Reduced_Mat_v3.csv",Tables.table(get_setting(m,:State2Control)))
            CSV.write("../../../../DSGE_P_Mat_v3.csv",Tables.table(get_setting(m,:PRightAll)))
            CSV.write("../../../../DSGE_A_Reduced_Mat_v3.csv",Tables.table(m[:A].value))
            CSV.write("../../../../DSGE_B_Reduced_Mat_v3.csv",Tables.table(m[:B].value))
=#
