...
function load_posterior_moments(m::AbstractDSGEModel;
                                cloud::Union{SMC.Cloud,Cloud,ParticleCloud} = ParticleCloud(m, 0),
    ...
    tex_labels = [detexify(parameters[i].tex_label) for i in 1:length(parameters)]
...
function sample_λ(m::PoolModel{S}, data::Matrix{S}, θ::Vector{S},
    ...
    update!(m, θ)
    loglik, λ_particles, λ_weights = filter(m, data; tuning = tuning)
    return λ_particles[size(data,2)][1,sample(Weights(λ_weights[:,end]))]
end
