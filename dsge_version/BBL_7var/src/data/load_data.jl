...
function load_data(m::AbstractDSGEModel; cond_type::Symbol = :none, try_disk::Bool = true,
    ...
    if recreate_data
        ...
        if :obs_nominalrate1 in cond_semi_names(m) || :obs_nominalrate1 in cond_full_names(m)
            ...
            dates = get_quarter_ends(iterate_quarters(date_mainsample_end(m), 1), date_conditional_end(m))
            n_cond = length(dates)
...
function construct_bluechip_data(m::AbstractModel, raw_forecasts::String, num_anticipated_shocks::Int, save_bluechip_rate_expectations_here::String)
    df = DataFrame(date = quarter_range(date_zlb_start(m), date_zlb_end(m)))
    for h = 1:num_anticipated_shocks
        df[!, Symbol("ant$h")] = NaN
    end
    for date in quarter_range(date_zlb_start(m), date_zlb_end(m))
...
function load_reference_forecast(m::AbstractModel, year::Int, quarter::Int, file::String, num_anticipated_shocks::Int)
    # Dates
    start_date = quartertodate(string(year)*"q"*string(quarter))
    end_date   = iterate_quarters(start_date, num_anticipated_shocks)
    dates      = quarter_range(start_date, end_date)
    ...
    horizon = minimum([horizon, subtract_quarters(get_setting(m, :date_zlb_end), quartertodate(string(year)*"q"*string(quarter))) + 1])
    for t = 1:horizon
